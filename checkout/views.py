...
from shop.models import Shoe, StockLevel
    ...
    if request.method=="POST":
        ...
        if order_form.is_valid():
            ...
            for line_id, line_info in cart.items():
                ...
                stock_level = get_object_or_404(StockLevel, size=size, shoe_model=product_id)
                quantity = line_info['quantity']
                if stock_level.stock < quantity:
                    messages.error("Oh no there's only {0} left! Please adjust the quantity in your cart.")
